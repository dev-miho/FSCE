...
if __name__ == '__main__':
    ...
    train_x=[[float(el) for el in row]for row in train_x]
    test_set=dataset[int(len(dataset)*0.85):]
    test_x=[row[:-1] for row in test_set]
    test_y=[row[-1] for row in test_set]
    test_x=[[float(el) for el in row]for row in test_x]
    ...
    sample=[float(el) for el in sample]
    pred_class=classifier.predict([sample])
    pred_proba=classifier.predict_proba([sample])
