...
if __name__ == "__main__":
    ...
    # Remove the column specified by col_index from the dataset
    for i in range(len(dataset)):
        new_row = []
        for index, element in enumerate(dataset[i]):
            if index != col_index:
    ...
    # Prepare the new sample by removing the specified column
    new_sample = []
    for index, element in enumerate(sample):
        if index != col_index:
            new_sample.append(element)

    # Predict class and probabilities for the new sample
    predict = classifier.predict([new_sample])
    pred_proba = classifier.predict_proba([new_sample])
