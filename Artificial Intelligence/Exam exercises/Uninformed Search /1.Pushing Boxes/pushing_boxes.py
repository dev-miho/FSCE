...
class Box(Problem):
    ...
    def goal_test(self, state):
        ...
        return first_box == (4,4) and second_box == (4,4)

    @staticmethod
    def check_valid(state):
        man_position,first_box,second_box=state
        
        if man_position[0] < 0 or man_position[0] > 4 or man_position[1] < 0 or man_position[1] > 4:
            return False
        
        if first_box[0]<0 or first_box[0]>4 or first_box[1]<0 or first_box[1]>4:
            return False
        
        if second_box[0] < 0 or second_box[0] > 4 or second_box[1] < 0 or second_box[1] > 4:
            return False
        if first_box == second_box and first_box != (4,4):
            return False 
    ...
    def successor(self, state):
        ...
        for direction,(x,y) in directions.items():
            new_position=(man_position[0]+x,man_position[1]+y)
            
            
            if new_position != first_box and new_position != second_box:
                if self.check_valid((new_position,first_box,second_box)):
                    succ[f"Move man {direction}"]=(new_position,first_box,second_box)
            
            if new_position == first_box:
                new_box=(first_box[0]+x,first_box[1]+y)
                if self.check_valid((new_position,new_box,second_box)):
                    succ[f"Push box 1 {direction}"]=(new_position,new_box,second_box)
            
            if new_position == second_box:
                new_box=(second_box[0]+x,second_box[1]+y)
                if self.check_valid((new_position,first_box,new_box)):
                    succ[f"Push box 2 {direction}"]=(new_position,first_box,new_box)
