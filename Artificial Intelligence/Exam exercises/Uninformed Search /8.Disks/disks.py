...
class Disks(Problem):
    ...
    def successor(self, state):
        succ = {}
        directions = [("R1",1), ("R2",2), ("L1",-1), ("L2",-2)]
        for i in range(len(state)):
            disk=state[i]
            if disk==-1:
                continue
            for direction,x in directions:
                new_list=list(state)
                new_pos=i+x
                if x==1 and new_pos<len(state) and state[new_pos] == -1:
                    new_list[i],new_list[new_pos]=new_list[new_pos],new_list[i]
                    succ[direction+": Disk "+str(disk)]=tuple(new_list)
                elif x==2 and new_pos<len(state) and state[new_pos] == -1 and state[i+1] != -1:
                    new_list[i],new_list[new_pos]=new_list[new_pos],new_list[i]
                    succ[direction+": Disk "+str(disk)]=tuple(new_list)
                elif x==-1 and new_pos>=0 and state[new_pos] == -1:
                    new_list[i],new_list[new_pos]=new_list[new_pos],new_list[i]
                    succ[direction+": Disk "+str(disk)]=tuple(new_list)
                elif x==-2 and new_pos>=0 and state[new_pos] == -1 and state[i-1] != -1:
                    new_list[i],new_list[new_pos]=new_list[new_pos],new_list[i]
                    succ[direction+": Disk "+str(disk)]=tuple(new_list)
        return succ
...
if __name__ == "__main__":
    N=int(input())
    L=int(input())
    disks=[i+1 if i+1<=N else -1 for i in range(L)]
    goal=[]
    for i in range(L-N):
        goal.append(-1)
    for i in range(N):
    ...
    problem=Disks(tuple(disks),tuple(goal))
