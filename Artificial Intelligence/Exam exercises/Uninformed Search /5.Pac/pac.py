...
def change_dicrection(d,action):
    if d == "east":
        if action == "forward":
            return "east"
...
class Pac(Problem):
    ...
    def goal_test(self, state):
        _,_, dots = state
        return len(state[2]) == self.goal
    
    def successor(self, state):
        succ={}
        
        pac_pos, direction, dots = state
        ...
        if direction == "east":
            ...
            dirs={"forward":(0,1),"backward":(0,-1),"left":(-1,0),"right":(1,0)}
        else:
            dirs={"forward":(0,-1),"backward":(0,1),"left":(1,0),"right":(-1,0)}
        
        for dir,(x,y) in dirs.items():
            new_pac_pos=(pac_pos[0]+x,pac_pos[1]+y)
            if self.check_valid((new_pac_pos, direction, dots)):
                word = "Move" if dir in ["forward", "backward"] else "Turn"
                new_direction=change_dicrection(direction,dir)
                if new_pac_pos not in dots:
                    succ[f"{word} {dir}"]=(new_pac_pos, new_direction, dots)
                else:
                    new_dots=list(dots)
                    new_dots.remove(new_pac_pos)
                    new_dots=tuple(new_dots)
                    succ[f"{word} {dir}"]=(new_pac_pos, new_direction, new_dots)       
        return succ
    
    @staticmethod
    def check_valid(state):
        pac_pos,direction,dots=state
        if pac_pos in obstacles:
            return False
        if pac_pos[0]<0 or pac_pos[0]>9 or pac_pos[1]<0 or pac_pos[1]>9:
            return False
        return True

if __name__ == "__main__":
    
    obstacles = [(0,9),(1,9),(2,9),(3,9),(6,9),(0,8),(8,8),(9,8),
                 (4,7),(8,7),(9,7),(0,6),(4,6),(3,6),(5,6),(4,5),
                 (1,4),(8,4),(9,4),(1,3),(1,2),(6,2),(4,1),(5,1),
                 (6,1),(8,1),(6,0)]
    ...
    initial_state=(ball_pos,direction,dots)
    
    problem=Pac(initial_state,obstacles,0)
